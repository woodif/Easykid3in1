...
void noTone(unsigned int BUZZER1)
{
    ledcDetachPin(BUZZER1);
    ledcWrite(TONE_CHANNEL, 0);
}
...
void tone(unsigned int frequency, unsigned long duration)
{
    ...
    ledcAttachPin(BUZZER, TONE_CHANNEL);
...
void tone(unsigned int BUZZER1, unsigned int frequency, unsigned long duration)
{
    if (ledcRead(TONE_CHANNEL)) {
        log_e("Tone channel %d is already in use", ledcRead(TONE_CHANNEL));
        return;
    }
    ledcAttachPin(BUZZER1, TONE_CHANNEL);
    ledcWriteTone(TONE_CHANNEL, frequency);
    if (duration) {
        delay(duration);
        noTone();
